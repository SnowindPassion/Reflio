const ReflioDomainRoot="http://localhost:3000",ReflioAPIRoot=ReflioDomainRoot+"/api/v1",rootDomain=window.location.host,queryString=window.location.search,urlParams=new URLSearchParams(queryString),reflioVerifyParam=urlParams.get("reflioVerify"),reflioReferralParam=urlParams.get("via")?urlParams.get("via"):urlParams.get("ref")?urlParams.get("ref"):null;let reflioReferralObject=localStorage.getItem("reflioReferral");const reflioInnerScript=document.querySelector("script[data-reflio]");let scrolledPercentage=((document.documentElement.scrollTop+document.body.scrollTop)/(document.documentElement.scrollHeight-document.documentElement.clientHeight)*100).toFixed(0);class rfl{details(){return{companyId:reflioInnerScript.getAttribute("data-reflio")?reflioInnerScript.getAttribute("data-reflio"):null,rootDomain:rootDomain,domains:reflioInnerScript.getAttribute("data-domains")?reflioInnerScript.getAttribute("data-domains"):null,hidePopup:!!reflioInnerScript.getAttribute("hidePopup"),consentBypass:!!reflioInnerScript.getAttribute("consentBypass")}}async checkDomainVerification(){return fetch(ReflioAPIRoot+"/verify-company").then(function(e){return e.json()})}async impression(e,o){return e&&o||console.warn("Reflio: could not track impression. Referral code / companyId not found."),fetch(ReflioAPIRoot+"/record-impression",{method:"POST",body:JSON.stringify({referralCode:e,companyId:o})}).then(function(e){return e.json()})}async campaignDetails(e,o){return e&&o||console.warn("Reflio: could not get campaign details. Referral code / companyId not found."),fetch(ReflioAPIRoot+"/campaign-details",{method:"POST",body:JSON.stringify({referralCode:e,companyId:o})}).then(function(e){return e.json()})}checkCookie(){var o="reflioData=";let e=decodeURIComponent(document.cookie);var t=e.split(";");for(let i=0;i<t.length;i++){let e=t[i];for(;" "==e.charAt(0);)e=e.substring(1);if(0==e.indexOf(o))try{return JSON.parse(e.substring(o.length,e.length))}catch(e){return{error:!0}}}return null}deleteCookie(){return document.cookie="reflioData=; Path=/; Expires=Thu, 01 Jan 1970 00:00:01 GMT;","cookie_deleted"}consentRequired(){return!0!==Reflio.details().consentBypass&&!!(Intl.DateTimeFormat().resolvedOptions().timeZone&&0<=Intl.DateTimeFormat().resolvedOptions().timeZone.indexOf("Europe")&&!1===Reflio.details().consentBypass)}cookieEligible(){return null!==reflioReferralParam&&null!==Reflio.details().companyId&&null===Reflio.checkCookie()}cookieExists(){return null!==Reflio.checkCookie()}consentCleanup(){document.getElementById("reflio-confirm-modal")&&document.getElementById("reflio-confirm-modal").parentNode.removeChild(document.getElementById("reflio-confirm-modal")),document.getElementById("reflio-confirm")&&document.getElementById("reflio-confirm").parentNode.removeChild(document.getElementById("reflio-confirm")),document.getElementById("reflio-confirm-styles")&&document.getElementById("reflio-confirm-styles").parentNode.removeChild(document.getElementById("reflio-confirm-styles")),document.getElementById("reflio-consent")&&document.getElementById("reflio-consent").parentNode.removeChild(document.getElementById("reflio-consent"))}async register(){var e;!0===Reflio.cookieEligible()&&(document.getElementById("reflio-confirm-button")&&(document.getElementById("reflio-confirm-button").innerText="Loading..."),(e=await Reflio.impression(reflioReferralParam,Reflio.details().companyId))?.referral_details&&Reflio.details().domains&&(document.querySelectorAll("[href]").forEach(i=>{Reflio.details().domains?.includes(",")&&Reflio.details().domains.split(",").map(e=>{if(i.href?.includes(e.trim())&&!i.href.includes(Reflio.details().rootDomain)){let e=new URL(i.href),o=e.searchParams;o.set("ref",reflioReferralParam),e.search=o.toString();var t=e.toString();i.href=t}})}),e?.referral_details?document.cookie=`reflioData=${JSON.stringify(e?.referral_details)}; expires=`+e?.referral_details?.cookie_date:Reflio.consentCleanup()))}async showPopup(){if(localStorage.getItem("reflioNoConsent"))return!1;const o=await Reflio.campaignDetails(reflioReferralParam,Reflio.details().companyId);if(!o?.campaign_details)return!1;var e=`
    ...
    `;if(document.getElementById("reflio-confirm")||(document.body.innerHTML+=e),setTimeout(()=>{document.getElementById("reflio-confirm-modal").classList.add("reflio-appear")},500),!document.getElementById("reflio-confirm-styles")){let e=document.createElement("style");e.type="text/css",e.innerText=`
      ...
      `,e.setAttribute("id","reflio-confirm-styles"),document.head.appendChild(e)}document.getElementById("reflio-confirm-button")&&document.getElementById("reflio-confirm-button").addEventListener("click",function(){var e;document.getElementById("reflio-buttons")&&(e=`
          ...
          `,document.getElementById("reflio-buttons").innerHTML=e,document.getElementById("reflio-content-subtitle")&&(document.getElementById("reflio-content-subtitle").innerText="Use the below discunt code at checkout:"),document.getElementById("reflio-discount-code-input")&&o?.campaign_details?.discount_code&&document.getElementById("reflio-discount-code-input").addEventListener("click",function(){document.getElementById("reflio-discount-code-input").select(),document.execCommand("copy"),document.getElementById("reflio-discount-code").innerHTML+=`
              ...
              `}),document.getElementById("reflio-close-button")&&document.getElementById("reflio-close-button").addEventListener("click",function(){Reflio.consentCleanup()})),Reflio.register()}),document.getElementById("reflio-cancel-button")&&document.getElementById("reflio-cancel-button").addEventListener("click",function(){Reflio.consentCleanup(),localStorage.setItem("reflioNoConsent",!0)})}async convert(e){if(!(e&&null!==Reflio.checkCookie()&&Reflio.checkCookie().referral_id&&Reflio.checkCookie().campaign_id&&Reflio.checkCookie().affiliate_id&&Reflio.checkCookie().cookie_date))return console.warn("Reflio: Conversion could not be tracked."),!1;var o=await fetch(ReflioAPIRoot+"/convert-referral",{method:"POST",body:JSON.stringify({referralId:Reflio.checkCookie().referral_id,campaignId:Reflio.checkCookie().campaign_id,affiliateId:Reflio.checkCookie().affiliate_id,cookieDate:Reflio.checkCookie().cookie_date,email:e})}).then(function(e){return e.json()});return"error"!==o?.conversion_details&&Reflio.deleteCookie(),o}}const Reflio=new rfl;function activatePopup(){33<=scrolledPercentage&&!1===Reflio.details().hidePopup&&!0===Reflio.consentRequired()&&!1===Reflio.cookieExists()&&setTimeout(()=>{Reflio.showPopup()},2e3)}null!==reflioVerifyParam&&"true"===reflioVerifyParam&&Reflio.checkDomainVerification().then(e=>{!0===e?.verified&&(console.log("Domain was verified:"),console.log("Redirect string: ",`${ReflioDomainRoot}/dashboard/${Reflio.details().companyId}/setup/verify`),window.location.href=`${ReflioDomainRoot}/dashboard/${Reflio.details().companyId}/setup/verify`)}),!1===Reflio.consentRequired()&&!0===Reflio.cookieEligible()&&Reflio.register(),activatePopup(),window.addEventListener("scroll",function e(){33<=(scrolledPercentage=((document.documentElement.scrollTop+document.body.scrollTop)/(document.documentElement.scrollHeight-document.documentElement.clientHeight)*100).toFixed(0))&&(window.removeEventListener("scroll",e,!1),activatePopup())},!1),console.log("consentBypass"),console.log(Reflio.details().consentBypass),!0===Reflio.cookieExists()&&Reflio.consentCleanup(),reflioInnerScript||console.error("Could not load Reflio: make sure the <script> tag includes data-reflio='<companyId>'");